...
# taken basically wholesale from https://github.com/python-xlib/python-xlib/issues/200
...
class OutlineWindow:
    def __init__(self, display, x, y, w, h, lw=3):
        ...
        bggc = self.screen.root.create_gc(
            foreground=0xFF0000, background=self.screen.black_pixel
        )
        ...
        self.window.shape_mask(
            shape.SO.Subtract, shape.SK.Bounding, x + lw, y + lw, inner_rect
        )
...
if __name__ == "__main__":
    ...
    match = re.match(r"([0-9]+)x([0-9]+)\+([0-9]+)\+([0-9]+)", args.spec)
    if match:
        w = int(match.group(1))
        h = int(match.group(2))
        x = int(match.group(3))
        y = int(match.group(4))
        ow = OutlineWindow(display.Display(), x, y, w, h, lw=1).loop()
